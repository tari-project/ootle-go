...
func TestSubmitSealed_SSEHappyPath(t *testing.T) {
	...
	c := NewClient(mock)
	result, err := c.SubmitSealed(context.Background(), EncodedPublicTransfer{EncodedTransaction: fakeEncodedTx})
	if err != nil {
...
func TestSubmitSealed_SSEPreferredOverPoll(t *testing.T) {
	...
	mock := &mockTransport{
		submit: func(context.Context, string) (string, error) { return fakeTxID, nil },
		stream: silentStream,
		result: func(context.Context, string) (json.RawMessage, bool, error) {
			return rawResult, true, nil
		},
	...
	start := time.Now()
	if _, err := c.SubmitSealed(context.Background(), EncodedPublicTransfer{EncodedTransaction: fakeEncodedTx}); err != nil {
		t.Fatalf("SubmitSealed: %v", err)
	}
...
func TestSubmitSealed_NoStreamerUsesPoll(t *testing.T) {
	...
	tr := &pollOnlyTransport{raw: rawResult, txID: fakeTxID}
...
func TestSubmitSealed_ContextCancelDuringSSE(t *testing.T) {
	...
	_, err := c.SubmitSealed(ctx, EncodedPublicTransfer{EncodedTransaction: fakeEncodedTx})
